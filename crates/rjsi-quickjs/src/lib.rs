...
use rquickjs::{Array, ArrayBuffer, CatchResultExt, CaughtError, Context, Ctx, Function as QjsFunction, Object, Persistent, Runtime, String as QjsString, Value};
...
impl QuickJsRuntimeContext {
    ...
    pub fn new() -> Self {
        ...
        Self { inner: Rc::new(QuickJsRuntimeInner { owner_thread: thread::current().id(), _runtime: runtime, context }) }
    }

    fn assert_owner_thread(&self) -> JsResult<()> {
        if thread::current().id() != self.inner.owner_thread {
            return Err(HostError::new(rjsi_core::error::E_INVALID_STATE, "QuickJS runtime accessed from a non-owner thread").into());
        }
        Ok(())
    }
}
    ...
    F: HostFunction<QuickJsEngine> + 'js,
{
    ...
    fn call<'a>(&self, params: QjsParams<'a, 'js>) -> rquickjs::Result<Value<'js>> {
        let mut scope = QuickJsScope { ctx: params.ctx().clone() };
        let ctx = scope.ctx.clone();
        let mut accessor = ParamsAccessor::<QuickJsEngine>::new(&mut scope, QuickJsCallbackArgs { params });
        self.function
            .try_borrow_mut()
            .map_err(|_| rquickjs::Exception::throw_internal(&ctx, "host function already borrowed"))?
            .call(&mut accessor)
    ...
    'js: 'a,
{
    ...
    fn this(&self, _scope: &mut QuickJsScope<'js>) -> Option<QuickJsValue<'js>> {
        Some(QuickJsValue { value: self.params.this(), exception: false })
    }

    fn get(&self, _scope: &mut QuickJsScope<'js>, index: usize) -> Option<QuickJsValue<'js>> {
        self.params.arg(index).map(|value| QuickJsValue { value, exception: false })
    }
}
...
impl JsEngine for QuickJsEngine {
    ...
    fn name() -> &'static str { "quickjs" }
    fn version() -> String { "rquickjs-0.11".to_string() }
}

impl JsRuntime for QuickJsRuntimeContext {
    type Engine = QuickJsEngine;

    fn with_scope<R>(&self, f: impl for<'js> FnOnce(&mut QuickJsScope<'js>) -> JsResult<R>) -> JsResult<R> {
...
impl<'js> JsScope<'js> for QuickJsScope<'js> {
    ...
    fn eval(&mut self, source: Source) -> JsResult<QuickJsValue<'js>> {
        ...
        Ok(match self.ctx.eval::<Value<'js>, _>(code).catch(&self.ctx) {
    ...
    fn undefined(&mut self) -> QuickJsValue<'js> { self.from_value(Value::new_undefined(self.ctx.clone()), false) }
    fn null(&mut self) -> QuickJsValue<'js> { self.from_value(Value::new_null(self.ctx.clone()), false) }
    fn boolean(&mut self, value: bool) -> QuickJsValue<'js> { self.from_value(Value::new_bool(self.ctx.clone(), value), false) }
    fn number(&mut self, value: f64) -> QuickJsValue<'js> { self.from_value(Value::new_float(self.ctx.clone(), value), false) }
    ...
    fn host_function<F>(&mut self, _name: &'static str, function: F) -> Result<QuickJsValue<'js>, QuickJsValue<'js>>
    where
        F: HostFunction<QuickJsEngine>,
    {
        let ctx = self.ctx.clone();
        let function = QjsFunction::new(ctx.clone(), QuickJsHostAdapter { function: RefCell::new(function) })
    ...
    fn value_type(&mut self, value: &QuickJsValue<'js>) -> JsValueType {
        ...
        else if value.is_bool() { JsValueType::Boolean }
        ...
        else if value.is_error() { JsValueType::Error }
    ...
    fn to_string(&mut self, value: &QuickJsValue<'js>) -> Option<String> {
        if value.exception { return None; }
        value.value.clone().into_string().and_then(|s| s.to_string().ok())
    }

    fn property_key(&mut self, key: &str) -> QuickJsPropertyKey<'js> { QuickJsPropertyKey(std::borrow::Cow::Owned(key.to_owned())) }

    fn get_property(&mut self, object: &QuickJsValue<'js>, key: &QuickJsPropertyKey<'js>) -> Result<Option<QuickJsValue<'js>>, QuickJsValue<'js>> {
    ...
    fn set_property(&mut self, object: &QuickJsValue<'js>, key: &QuickJsPropertyKey<'js>, value: &QuickJsValue<'js>) -> Result<(), QuickJsValue<'js>> {
        value_to_object(object.value.clone())
            .and_then(|object| object.set(key.0.as_ref(), value.value.clone()))
            .map_err(|err| self.from_error(err))
    }

    fn has_property(&mut self, object: &QuickJsValue<'js>, key: &QuickJsPropertyKey<'js>) -> Result<bool, QuickJsValue<'js>> {
        value_to_object(object.value.clone())
            .and_then(|object| object.contains_key(key.0.as_ref()))
            .map_err(|err| self.from_error(err))
    }

    fn delete_property(&mut self, object: &QuickJsValue<'js>, key: &QuickJsPropertyKey<'js>) -> Result<bool, QuickJsValue<'js>> {
        value_to_object(object.value.clone())
            .and_then(|object| object.remove(key.0.as_ref()).map(|_| true))
            .map_err(|err| self.from_error(err))
    }

    fn define_property(&mut self, object: &QuickJsValue<'js>, key: &QuickJsPropertyKey<'js>, value: &QuickJsValue<'js>, _attributes: PropertyAttributes) -> Result<(), QuickJsValue<'js>> {
        self.set_property(object, key, value)
    }

    fn get_index(&mut self, object: &QuickJsValue<'js>, index: u32) -> Result<Option<QuickJsValue<'js>>, QuickJsValue<'js>> {
        let key = QuickJsPropertyKey(index.to_string().into());
        self.get_property(object, &key)
    }

    fn set_index(&mut self, object: &QuickJsValue<'js>, index: u32, value: &QuickJsValue<'js>) -> Result<(), QuickJsValue<'js>> {
        let key = QuickJsPropertyKey(index.to_string().into());
        self.set_property(object, &key, value)
    }

    fn call_function(&mut self, function: &QuickJsValue<'js>, _this: Option<&QuickJsValue<'js>>, argv: &[QuickJsValue<'js>]) -> Result<QuickJsValue<'js>, QuickJsValue<'js>> {
        let function = function.value.clone().into_function().ok_or_else(|| rquickjs::Exception::throw_type(&self.ctx, "Value is not callable"));
        let args = argv.iter().map(|arg| arg.value.clone()).collect::<Vec<_>>();
        match function.and_then(|function| function.call::<_, Value<'js>>((Rest(args),))).catch(&self.ctx) {
...
impl JsGlobalHandle<QuickJsEngine> for QuickJsGlobal {
    fn new<'js>(scope: &mut <QuickJsEngine as JsEngine>::Scope<'js>, value: &<QuickJsEngine as JsEngine>::Value<'js>) -> Self {
        Self { handle: Persistent::save(&scope.ctx, value.value.clone()), exception: value.exception }
    }

    fn get<'js>(&self, scope: &mut <QuickJsEngine as JsEngine>::Scope<'js>) -> <QuickJsEngine as JsEngine>::Value<'js> {
        let value = self.handle.clone().restore(&scope.ctx).unwrap_or_else(|_| Value::new_undefined(scope.ctx.clone()));
        QuickJsValue { value, exception: self.exception }
    }
}

fn value_to_object<'js>(value: Value<'js>) -> Result<Object<'js>, rquickjs::Error> {
    let ctx = value.ctx().clone();
    value.into_object().ok_or_else(|| rquickjs::Exception::throw_type(&ctx, "Value is not an object"))
}
