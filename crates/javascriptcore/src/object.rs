...
use crate::{JSException, JSObject, JSString, JSValue, sys};
...
impl JSObject {
    ...
    ///
    /// * `name`: A value that can be converted to a [`JSString`] containing the
    ///   property's name.
    ///
    ...
    /// ```
    ///
    /// This also works with objects when the keys are strings of numeric
    /// indexes:
    ///
    /// ```
    /// # use javascriptcore::{JSContext, JSObject, JSString, JSValue};
    ...
    ///
    /// This can be used to create a new property, or to update an existing
    /// property.
    ///
    ...
    /// * `value`: A value containing the property's value.
    ///
    /// Calling `set_property_at_index` is equivalent to calling `set_property`
    /// with a string containing `index`, but `set_property_at_index`
