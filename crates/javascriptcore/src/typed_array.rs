...
use crate::{JSException, JSObject, JSTypedArray, JSTypedArrayType, JSValue, sys};
...
impl JSTypedArray {
    ...
    /// Typed Array type, or `JSTypedArrayType::None` if the value is not a
    /// Typed Array object.
    ///
    /// ```rust
    /// # use javascriptcore::*;
    ...
    /// another Typed Array, it's a “subview” that can start from an offset, up
    /// to a certain length, which is the byte length.
    ///
    /// ```rust
    /// # use javascriptcore::*;
    /// let ctx = JSContext::default();
    ...
    /// # Safety
    ///
    /// The pointer of the slice returned by this function is temporary and is
    /// not guaranteed to remain valid across JavaScriptCore API calls.
    ///
    /// # Example
    ///    
    /// ```rust
...
mod tests {
    use super::*;
    use crate::{JSContext, evaluate_script};
