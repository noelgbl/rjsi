...
use crate::{
    JSClass, JSContext, JSException, JSObject, JSString, JSType, JSTypedArray, JSTypedArrayType, JSValue, sys
};
...
impl JSValue {
    ...
    ///
    /// Returns a `JSValue` of the `boolean` type, representing the value of
    /// `boolean`.
    ///
    /// ```
    /// # use javascriptcore::*;
    ...
    ///
    /// Returns a `JSValue` of the `number` type, representing the value of
    /// `number`.
    ///
    /// ```
    /// # use javascriptcore::*;
    ...
    ///
    /// Returns a `JSValue` of the `string` type, representing the value of
    /// `string`.
    ///
    /// ```
    /// # use javascriptcore::*;
    ...
    ///
    /// Returns a `JSValue` of the `symbol` type, whose description matches the
    /// one provided.
    ///
    /// ```
    /// # use javascriptcore::*;
    ...
    ///
    /// Returns a `JSValue` of the `array` type, otherwise an
    ...
    ///
    /// * `ctx`: The execution context to use.
    /// * `bytes`: The typed array bytes. The constructed `TypedArray` doesn't
    ///   copy the bytes, thus this method takes a `&mut` reference as it is
    ///   possible to mutate the bytes via `TypedArray` or via Rust.
    ///
    /// Returns a `JSValue` of the `TypedArray` type, otherwise an
    /// [exception](JSException).
    ///
    /// # Safety
    ///
    /// `bytes` can be mutated both by Rust or JavaScript. There is no lock, no
    /// mutex, no guard. Be extremely careful when using this API. `bytes`
    /// aren't copied, they are borrowed mutably by JavaScript. Dropping the
    /// value in Rust will clear them in JavaScript, and vice versa. Hence,
    ...
    /// Creates a JavaScript function where the function implementation is
    /// written in Rust.
    ///
    /// * `ctx`: The execution context to use.
    ...
    ///
    /// Returns an `Option` with the `JSValue` containing the parsed value, or
    ...
    ///
    /// Returns either the numeric result of conversion, or an
    /// [exception](JSException) if one was thrown.
    ///
    /// ```
    /// # use javascriptcore::*;
    ...
    /// [exception](JSException) if one was thrown.  Ownership follows the
    /// Create Rule.
    ///
    /// ```
    /// # use javascriptcore::*;
    ...
    ///
    /// Returns either the `JSObject` result of conversion, or an
    /// [exception](JSException) if one was thrown.
    ///
    /// ```
    /// # use javascriptcore::*;
    ...
    /// resulting object.
    ///
    /// Returns either the `JSTypedArray` result of conversion, or an
    /// [exception](JSException) if one was thrown.
    ///
    /// ```rust
    /// # use javascriptcore::*;
