...
use crate::{JSClass, JSContext, JSException, JSObject, JSValue, sys};
...
impl JSClass {
    ...
    ///
    /// Note that it doesn't instantiate the class. To do a proper
    /// instantiation, one has to call [`JSObject::call_as_constructor`].
    ///
    /// ```rust
    /// # use javascriptcore::*;
    /// let ctx = JSContext::default();
...
impl JSClassBuilder<'_> {
    /// Set a class constructor, called by [the `new` operator in
    /// JavaScript][new].
    ///
    /// The easiest way to generate a [`JSObjectCallAsConstructorCallback`] is
    /// by using the [`crate::constructor_callback`] procedural macro.
    ///
