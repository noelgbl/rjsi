...
/// Names are added to the accumulator using
...
extern "C" {
    ...
    ///   source file. This is used by debuggers and when reporting exceptions.
    ///   Pass `NULL` if you do not care to include source file information.
    /// * `startingLineNumber`: An integer value specifying the script's
    ///   starting line number in the file located at `sourceURL`. This is only
    ///   used when reporting exceptions. The value is one-based, so the first
    ///   line is line `1` and invalid values are clamped to `1`.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store an
    ///   exception, if any. Pass `NULL` if you do not care to store an
    ///   exception.
    ///
    /// The [`JSValueRef`] that results from evaluating script, or `NULL` if an
    /// exception is thrown.
    ///
    /// # See also
    ///
    ...
    ///
    /// * `ctx`: The execution context to use.
    /// * `script`: A [`JSStringRef`] containing the script to check for syntax
    ///   errors.
    /// * `sourceURL`: A [`JSStringRef`] containing a URL for the script's
    ///   source file. This is only used when reporting exceptions. Pass `NULL`
    ///   if you do not care to include source file information in exceptions.
    /// * `startingLineNumber`: An integer value specifying the script's
    ///   starting line number in the file located at `sourceURL`. This is only
    ///   used when reporting exceptions. The value is one-based, so the first
    ///   line is line `1` and invalid values are clamped to `1`.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store a syntax
    ///   error exception, if any. Pass `NULL` if you do not care to store a
    ///   syntax error exception.
    ///
    /// Returns `true` if the script is syntactically correct, otherwise
    /// `false`.
    ///
    /// # See also
    ///
...
extern "C" {
    ...
    ///
    /// Returns `true` if `value`'s type is the `undefined` type, otherwise
    /// `false`.
    ...
    /// * `value`: The [`JSValueRef`] to test.
    ///
    /// Returns `true` if `value`'s type is the `boolean` type, otherwise
    /// `false`.
    ...
    /// * `value`: The [`JSValueRef`] to test.
    ///
    /// Returns `true` if `value`'s type is the `number` type, otherwise
    /// `false`.
    ...
    /// * `value`: The [`JSValueRef`] to test.
    ///
    /// Returns `true` if `value`'s type is the `string` type, otherwise
    /// `false`.
    ...
    /// * `value`: The [`JSValueRef`] to test.
    ///
    /// Returns `true` if `value`'s type is the `symbol` type, otherwise
    /// `false`.
    ...
    /// * `value`: The [`JSValueRef`] to test.
    ///
    /// Returns `true` if `value`'s type is the `object` type, otherwise
    /// `false`.
    pub fn JSValueIsObject(ctx: JSContextRef, value: JSValueRef) -> bool;

    /// Tests whether a JavaScript value is an `object` with a given class in
    /// its class chain.
    ///
    /// * `ctx`: The execution context to use.
    /// * `value`: The [`JSValueRef`] to test.
    ...
    ///
    /// Returns a [`JSValueRef`] of the `boolean` type, representing the value
    /// of `boolean`.
    ...
    ///
    /// Returns a [`JSValueRef`] of the `number` type, representing the value of
    /// `number`.
    ...
    ///
    /// Returns a [`JSValueRef`] of the `string` type, representing the value of
    /// `string`.
    ...
    ///   [`JSValueRef`].
    ///
    /// Returns a [`JSValueRef`] of the `symbol` type, whose description matches
    /// the one provided.
    ...
    ///
    /// Returns a [`JSValueRef`] containing the parsed value, or `NULL` if the
    /// input is invalid.
    pub fn JSValueMakeFromJSONString(ctx: JSContextRef, string: JSStringRef) -> JSValueRef;

    /// Creates a JavaScript string containing the JSON serialized
    /// representation of a JS value.
    ///
    /// * `ctx`: The execution context to use.
    /// * `value`: The value to serialize.
    /// * `indent`: The number of spaces to indent when nesting. If `0`, the
    ///   resulting JSON will not contains newlines. The size of the indent is
    ///   clamped to `10` spaces.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store an
    ///   exception, if any. Pass `NULL` if you do not care to store an
    ///   exception.
    ///
    /// Returns a [`JSStringRef`] with the result of serialization, or `NULL` if
    ...
    /// Converts a JavaScript value to boolean and returns the resulting
    /// boolean.
    ///
    /// * `ctx`: The execution context to use.
    ...
    ///   exception.
    ///
    /// Returns the numeric result of conversion, or `NaN` if an exception is
    /// thrown.
...
/// * `propertyName`: A [`JSStringRef`] containing the name of the property look
///   up.
///
...
/// * `ctx`: The execution context to use.
/// * `object`: The [`JSObjectRef`] to search for the property.
/// * `propertyName`: A [`JSStringRef`] containing the name of the property to
///   get.
/// * `exception`: A pointer to a [`JSValueRef`] in which to return an
///   exception, if any.
///
...
///   exception, if any.
///
/// Returns `true` if `propertyName` was successfully deleted, otherwise
/// `false`.
///
...
///
/// If you named your function `GetPropertyNames`, you would declare it like
/// this:
///
/// ```ignore
/// void
...
/// * `thisObject`: A [`JSObjectRef`] that is the `this` variable in the
///   function's scope.
/// * `argumentCount`: An integer count of the number of arguments in
///   `arguments`.
/// * `arguments`: A [`JSValueRef`] array of the arguments passed to the
///   function.
/// * `exception`: A pointer to a [`JSValueRef`] in which to return an
///   exception, if any.
///
...
///
/// If you named your function `CallAsConstructor`, you would declare it like
/// this:
///
/// ```ignore
/// JSObjectRef
...
///
/// * `ctx`: The execution context to use.
/// * `constructor`: The [`JSObjectRef`] that is the target of the `instanceof`
///   expression.
/// * `possibleInstance`: The [`JSValueRef`] being tested to determine if it is
...
///   exception, if any.
///
///Returns the objects' converted value, or `NULL` if the object was not
/// converted.
///
...
pub struct JSStaticValue {
    ...
    /// A logically ORed set of [`JSPropertyAttributes`] to give to the
    /// property.
...
///
/// All fields other than the version field are optional. Any pointer may be
/// `NULL`.
///
...
/// `SetX`, you would declare a [`JSStaticValue`] array containing `"X"` like
/// this:
///
/// ```ignore
...
pub struct JSClassDefinition {
    ...
    /// A [`JSClassRef`] to set as the class's parent class. Pass `NULL` use the
    ...
    /// The callback invoked when collecting the names of an object's
    /// properties.
...
extern "C" {
    ...
    ///
    /// * `ctx`: The execution context to use.
    /// * `jsClass`: The [`JSClassRef`] to assign to the object. Pass `NULL` to
    ...
    ///   used when converting the function to string. Pass `NULL` to create an
    ///   anonymous function.
    /// * `callAsFunction`: The [`JSObjectCallAsFunctionCallback`] to invoke
    ///   when the function is called.
    ///
    /// Returns a [`JSObjectRef`] that is a function. The object's prototype
    ...
    ///
    /// * `ctx`: The execution context to use.
    /// * `jsClass`: A [`JSClassRef`] that is the class your constructor will
    ///   assign to the objects its constructs. `jsClass` will be used to set
    ///   the constructor's `.prototype` property, and to evaluate `instanceof`
    ///   expressions. Pass `NULL` to use the default object class.
    /// * `callAsConstructor` A [`JSObjectCallAsConstructorCallback`] to invoke
    ///   when your constructor is used in a `new` expression. Pass `NULL` to
    ///   use the default object constructor.
    ///
    ...
    /// * `argumentCount`: An integer count of the number of arguments in
    ///   `arguments`.
    /// * `arguments`: A [`JSValueRef`] array of data to populate the `Array`
    ///   with. Pass `NULL` if `argumentCount` is `0`.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store an
    ///   exception, if any. Pass `NULL` if you do not care to store an
    ///   exception.
    ///
    ...
    /// * `argumentCount`: An integer count of the number of arguments in
    ///   `arguments`.
    /// * `arguments`: A [`JSValueRef`] array of arguments to pass to the `Date`
    ///   constructor. Pass `NULL` if `argumentCount` is `0`.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store an
    ///   exception, if any. Pass `NULL` if you do not care to store an
    ///   exception.
    ///
    ...
    /// * `argumentCount`: An integer count of the number of arguments in
    ///   `arguments`.
    /// * `arguments`: A [`JSValueRef`] array of arguments to pass to the
    ///   `Error` constructor. Pass `NULL` if `argumentCount` is `0`.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store an
    ///   exception, if any. Pass `NULL` if you do not care to store an
    ///   exception.
    ///
    ...
    ///   `arguments`.
    /// * `arguments`: A [`JSValueRef`] array of arguments to pass to the
    ///   `RegExp` constructor. Pass `NULL` if `argumentCount` is `0`.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store an
    ///   exception, if any. Pass `NULL` if you do not care to store an
    ///   exception.
    ///
    ...
    ///   resolve function for the new promise. Pass `NULL` if you do not care
    ///   to store the resolve callback.
    ...
    ///   `parameterNames`.
    /// * `parameterNames`: A [`JSStringRef`] array containing the names of the
    ///   function's parameters. Pass `NULL` if `parameterCount` is `0`.
    /// * `body`: A [`JSStringRef`] containing the script to use as the
    ///   function's body.
    /// * `sourceURL` A [`JSStringRef`] containing a URL for the script's source
    ///   file. This is only used when reporting exceptions. Pass `NULL` if you
    ///   do not care to include source file information in exceptions.
    /// * `startingLineNumber`: An integer value specifying the script's
    ///   starting line number in the file located at `sourceURL`. This is only
    ///   used when reporting exceptions. The value is one-based, so the first
    ///   line is line `1` and invalid values are clamped to `1`.
    ...
    /// * `propertyName`: A [`JSStringRef`] containing the property's name.
    /// * `value`: A [`JSValueRef`] to use as the property's value.
    /// * `attributes`: A logically ORed set of [`JSPropertyAttributes`] to give
    ///   to the property.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store an
    ///   exception, if any. Pass `NULL` if you do not care to store an
    ///   exception.
    ...
    /// Tests whether an object has a given property using a [`JSValueRef`] as
    /// the property key.
    ///
    /// * `ctx`: The execution context to use.
    /// * `object`: The [`JSObjectRef`] to test.
    /// * `propertyKey`: A [`JSValueRef`] containing the property key to use
    ///   when looking up the property.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store an
    ///   exception, if any. Pass `NULL` if you do not care to store an
    ///   exception.
    ///
    ...
    ///
    /// This function is the same as performing `propertyKey in object` from
    /// JavaScript.
    ///
    /// # See also
    ///
    ...
    /// Gets a property from an object using a [`JSValueRef`] as the property
    /// key.
    ///
    /// * `ctx`: The execution context to use.
    /// * `object`: The [`JSObjectRef`] whose property you want to get.
    ...
    ///   exception.
    ///
    /// The property's value if object has the property key, otherwise the
    /// undefined value.
    ///
    /// This function is the same as performing `object[propertyKey]` from
    /// JavaScript.
    ///
    /// # See also
    ///
    ...
    /// Deletes a property from an object using a [`JSValueRef`] as the property
    /// key.
    ///
    /// * `ctx`: The execution context to use.
    /// * `object`: The [`JSObjectRef`] whose property you want to delete.
    ...
    ///
    /// The default object class does not allocate storage for private data.
    /// Only objects created with a non-`NULL` [`JSClassRef`] can store private
    /// data.
    ///
    /// # See also
    ///
    ...
    /// * `object`: The [`JSObjectRef`] to test.
    ///
    /// Returns `true` if the object can be called as a function, otherwise
    /// `false`.
    ///
    /// # See also
    ///
    ...
    ///   `arguments`.
    /// * `arguments`: A [`JSValueRef`] array of arguments to pass to the
    ///   function. Pass `NULL` if `argumentCount` is `0`.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store an
    ///   exception, if any. Pass `NULL` if you do not care to store an
    ///   exception.
    ///
    /// Returns the [`JSValueRef`] that results from calling `object` as a
    /// function, or `NULL` if an exception is thrown or `object` is not a
    /// function.
    ///
    /// # See also
    ///
    ...
    /// * `object`: The [`JSObjectRef`] to test.
    ///
    /// Returns `true` if the object can be called as a constructor, otherwise
    /// `false`.
    ...
    ///   exception.
    ///
    /// Returns the [`JSObjectRef`] that results from calling `object` as a
    /// constructor, or `NULL` if an exception is thrown or `object` is not
    /// a constructor.
    ...
    /// Gets a property name at a given index in a JavaScript property name
    /// array.
    ///
    ...
    ///
    /// * `accumulator`: The accumulator object to which to add the property
    ///   name.
    ...
    ///
    /// * `globalObjectClass`: The class to use when creating the global object.
    ...
    ///
    /// * `group`: The context group to use. The created global context retains
    ///   the group.  Pass `NULL` to create a unique group for the context.
    /// * `globalObjectClass`: The class to use when creating the global object.
    ///   Pass NULL to use the default object class.
    ///
...
extern "C" {
    ...
    ///   [`JSStringRef`].
    /// * `numChars`: The number of characters to copy from the buffer pointed
    ///   to by `chars`.
    ///
    ...
    ///
    /// * `string`: The null-terminated UTF8 string to copy into the new
    ///   [`JSStringRef`].
    ///
    ...
    ///
    /// * `string`: The [`JSStringRef`] whose length (in Unicode characters) you
    ///   want to know.
    ///
    ...
    ///   null-terminated UTF8 representation of `string`. On return, `buffer`
    ///   contains a UTF8 string representation of `string`. If `bufferSize` is
    ///   too small, `buffer` will contain only partial results. If `buffer` is
    ///   not at least `bufferSize` bytes in size, behavior is undefined.
    /// * `bufferSize`: The size of the external buffer in bytes.
    ///
    /// Returns the number of bytes written into buffer (including the
    ...
    /// Creates a JavaScript Typed Array object with the given number of
    /// elements.
    ///
    /// * `ctx`: The execution context to use.
    /// * `arrayType`: A value identifying the type of array to create. If
    ///   `arrayType` is `JSTypedArrayType::None` or
    ...
    ///   `arrayType` is `JSTypedArrayType::None` or
    ///   `JSTypedArrayType::ArrayBuffer` then `NULL` will be returned.
    /// * `buffer`: An Array Buffer object that should be used as the backing
    ///   store for the created JavaScript Typed Array object.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store an
    ///   exception, if any. Pass `NULL` if you do not care to store an
    ///   exception.
    ///
    ...
    ///
    /// * `ctx`: The execution context to use.
    /// * `object`: The [`JSObjectRef`] whose Typed Array type data pointer to
    ///   obtain.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store an
    ///   exception, if any. Pass `NULL` if you do not care to store an
    ///   exception.
    ...
    /// * `byteLength`: The number of bytes pointed to by the parameter `bytes`.
    /// * `bytesDeallocator`: The allocator to use to deallocate the external
    ///   buffer when the Typed Array data object is deallocated.
    /// * `deallocatorContext`: A pointer to pass back to the deallocator.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store an
    ///   exception, if any. Pass `NULL` if you do not care to store an
    ///   exception.
    ...
    ///
    /// * `ctx`: The execution context to use.
    /// * `object`: The Array Buffer object whose internal backing store pointer
    ///   to return.
    /// * `exception`: A pointer to a [`JSValueRef`] in which to store an
    ///   exception, if any. Pass `NULL` if you do not care to store an
    ///   exception.
