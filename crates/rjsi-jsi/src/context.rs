...
impl<'rt, E: Engine> Context<'rt, E> {
    ...
    pub fn eval<'cx>(&'cx mut self, src: &str) -> JsResult<'cx, E, Value<'cx, E>> {
        E::eval(&mut self.raw, src, None).map(Value::new)
    }

    pub fn globals<'cx>(&'cx mut self) -> Object<'cx, E> {
        Object::new(E::global_object(&mut self.raw))
    }

    pub fn new_object<'cx>(&'cx mut self) -> JsResult<'cx, E, Object<'cx, E>> {
        E::object_new(&mut self.raw).map(Object::new)
    }

    pub fn undefined<'cx>(&'cx mut self) -> Value<'cx, E> {
        Value::new(E::make_undefined(&mut self.raw))
    }

    pub fn null<'cx>(&'cx mut self) -> Value<'cx, E> {
        Value::new(E::make_null(&mut self.raw))
    }

    pub fn boolean<'cx>(&'cx mut self, v: bool) -> Value<'cx, E> {
        Value::new(E::make_bool(&mut self.raw, v))
    }

    pub fn integer<'cx>(&'cx mut self, v: i32) -> Value<'cx, E> {
        Value::new(E::make_i32(&mut self.raw, v))
    }

    pub fn number<'cx>(&'cx mut self, v: f64) -> Value<'cx, E> {
        Value::new(E::make_f64(&mut self.raw, v))
    }

    pub fn string<'cx>(&'cx mut self, s: &str) -> JsResult<'cx, E, Value<'cx, E>> {
