use rjsi::{DefaultRuntime, FromJs, IntoJs, Runtime, ToJs, Value};
...
fn main() {
    ...
    let (greeting, point_sum, point_from_js, person_from_js) = runtime.with_scope(|cx| {
        ...
        globals.set(cx, "person", Value::new(person_value)).unwrap();
        ...
        globals.set(cx, "point", Value::new(point_value)).unwrap();
        ...
        let point_from_js = Point::from_js(cx, point_from_js_value.into_raw()).unwrap();

        let person_from_js_value = cx.eval("({ name: 'Grace', age: 35 })").unwrap();
        let person_from_js = Person::from_js(cx, person_from_js_value.into_raw()).unwrap();
